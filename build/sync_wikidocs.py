...
def student_front(text):
    return text.replace(
        "- **💬 모범 답안**: 토론 질문·심화 토론·실습 항목에는 모범 답안(예시 답안)을 음영 박스로 제공한다. 모범 답안은 '정답'이 아니라 논증의 예시이며, 수업에서는 이를 넘어서는 답을 기대한다. **박스를 읽기 전에 반드시 스스로 답을 구성해 볼 것.**",
        "- **💬 모범 답안**: 토론 질문·심화 토론·실습 항목의 모범 답안(예시 답안)은 부록 「토론 쟁점 해설」에 모아 두었다. 모범 답안은 '정답'이 아니라 논증의 예시이며, 수업에서는 이를 넘어서는 답을 기대한다. **부록을 읽기 전에 반드시 스스로 답을 구성해 볼 것.**")

files = sorted(glob.glob(os.path.join(SRC, "*.md")))
assert len(files) == 16
appendix = ["이 부록은 각 장 토론 질문과 실습의 모범 답안(예시 답안)을 모아 정리한 것이다. 모범 답안은 '정답'이 아니라 논증의 예시이며, 스스로 답을 구성해 본 뒤 참고하는 것을 권한다.\n"]
...
patch(APPENDIX_ID, {"content": stylize(app_text, None, "토론 쟁점 해설", "각 장 토론 질문·실습의 모범 답안", label="부록 A")})
